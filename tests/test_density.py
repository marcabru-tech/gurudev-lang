...
def test_matrix_density():
    ...
    # Esperamos pelo menos 60 células populadas (10 categorias x 6 domínios)
    assert len(populadas) >= 60
...
def test_embeddings_loaded():
    ...
    assert hasattr(com_emb[0], 'embedding') and isinstance(com_emb[0].embedding, list)
    assert len(com_emb[0].embedding) == 10
